  ...
  if (!isConnected && !isAuthLoading) {
    return (
      <div className="min-h-screen bg-[#0a0514] flex flex-col items-center justify-center p-6 text-center">
        ...
        <p className="text-white/40 max-w-sm mb-10 font-medium">Please connect your WebAuth wallet to access your creator dashboard and manage your earnings.</p>
        <Button 
          onClick={login}
        ...
        <Link to="/" className="mt-8 text-white/20 hover:text-white transition-colors flex items-center gap-2 font-bold text-sm">
        ...
        <div className="container mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-3 bg-white/5 border border-white/10 rounded-full px-4 py-1.5">
              <div className={cn("h-8 w-8 rounded-full flex items-center justify-center text-[10px] font-bold overflow-hidden border border-white/20", user.color)}>
                {user.avatarImage ? (
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              ...
              <TabsContent value="analytics" className="space-y-8 mt-0 animate-in fade-in slide-in-from-bottom-4 duration-300">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <Card className="bg-[#130b21] border-white/10 text-white rounded-[24px] overflow-hidden group">
                    <CardHeader className="pb-2">
                      <CardDescription className="text-white/40 uppercase tracking-widest text-[10px] font-black">Settled TAB</CardDescription>
                  ...
                  <Card className="bg-[#130b21] border-white/10 text-white rounded-[24px] overflow-hidden group">
                    <CardHeader className="pb-2">
                      <CardDescription className="text-white/40 uppercase tracking-widest text-[10px] font-black">XPR Balance</CardDescription>
                  ...
                  <Card className="bg-[#130b21] border-white/10 text-white rounded-[24px] overflow-hidden group">
                    <CardHeader className="pb-2">
                      <CardDescription className="text-white/40 uppercase tracking-widest text-[10px] font-black">Network ID</CardDescription>
                      <CardTitle className="text-3xl font-black group-hover:text-orange-500 transition-colors italic">@{actor}</CardTitle>
                    </CardHeader>
                    <CardContent>
                      <div className="text-xs text-white/20 font-bold flex items-center gap-1">
                ...
                <div className="bg-[#130b21] border border-white/10 rounded-[32px] p-8 relative overflow-hidden">
                  ...
                  <div className="flex items-center justify-between mb-8 relative z-10">
                    ...
                    <Button variant="ghost" onClick={handleManualRefresh} className="text-white/40 hover:text-white gap-2">
                      <RefreshCw className={cn("h-4 w-4", isRefreshing && "animate-spin")} /> Refresh
                    </Button>
                  </div>
                  <div className="space-y-4 relative z-10">
                    <div className="text-center py-12 border-2 border-dashed border-white/5 rounded-3xl">
                      <p className="text-white/20 font-bold italic">No recent tipping activity detected on-chain.</p>
                    </div>
                  </div>
                </div>
              </TabsContent>
                  ...
                  <div className="lg:col-span-5 space-y-6">
                    <Card className="bg-gradient-to-br from-purple-900/40 to-[#0a0514] border-white/10 rounded-[40px] p-8 shadow-2xl relative overflow-hidden group border-t-purple-500/30">
                       <div className="absolute top-0 right-0 p-6">
                         <ShieldCheck className="h-6 w-6 text-purple-400 opacity-50" />
                       </div>
                       <div className="space-y-1 mb-10">
                         <p className="text-[10px] font-black uppercase tracking-[0.3em] text-white/40">Connected Wallet</p>
                         <h3 className="text-3xl font-black italic tracking-tighter text-white">@{actor}</h3>
                       </div>
                       
                       <div className="space-y-6">
                         <div className="p-5 bg-white/5 rounded-2xl border border-white/10">
                            <p className="text-[9px] font-black text-white/30 uppercase tracking-widest mb-1">TAB Network Token</p>
                            <div className="flex items-end gap-2">
                              <span className="text-4xl font-black text-orange-500">{Number(balances.tab).toLocaleString()}</span>
                              <span className="text-sm font-bold text-white/40 mb-1.5">TAB</span>
                            </div>
                         </div>
                         <div className="p-5 bg-white/5 rounded-2xl border border-white/10">
                            <p className="text-[9px] font-black text-white/30 uppercase tracking-widest mb-1">Proton Native</p>
                            <div className="flex items-end gap-2">
                              <span className="text-4xl font-black text-purple-400">{Number(balances.xpr).toLocaleString()}</span>
                              <span className="text-sm font-bold text-white/40 mb-1.5">XPR</span>
                            </div>
                         </div>
                       </div>

                       <div className="mt-8 flex items-center justify-between pt-6 border-t border-white/5 text-[10px] font-black uppercase tracking-widest text-white/20">
                          <span>Verified Non-Custodial</span>
                          <span className="text-green-500">Online</span>
                       </div>
                    </Card>
                    
                    <div className="bg-orange-500/5 border border-orange-500/20 rounded-[32px] p-8 space-y-4">
                  ...
                  <Card className="lg:col-span-7 bg-[#130b21] border-white/10 rounded-[40px] p-8 shadow-2xl">
                    ...
                    <div className="space-y-8">
                      <div className="space-y-4">
                        ...
                        <div className="relative">
                          ...
                          <Input 
                            ...
                            className="bg-white/5 border-white/10 h-14 rounded-2xl pl-10 font-bold text-lg focus:ring-purple-500"
                          />
                        </div>
                      </div>
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="flex gap-2">
                          <Input 
                            ...
                            className="flex-1 bg-white/5 border-white/10 h-14 rounded-2xl font-black text-xl focus:ring-orange-500"
                          />
                          <Select value={transferSymbol} onValueChange={setTransferSymbol}>
                            <SelectTrigger className="w-[100px] bg-white/5 border-white/10 h-14 rounded-2xl font-black text-lg">
                              <SelectValue />
                            </SelectTrigger>
                      ...
                      <Button 
                        ...
                        disabled={isSending}
                        className="w-full h-20 bg-white text-black hover:bg-orange-500 hover:text-white font-black text-xl rounded-3xl transition-all shadow-xl shadow-white/5 active:scale-95 group"
                      >
                        {isSending ? (
                          <div className="flex items-center gap-3">
              ...
              <TabsContent value="card" className="mt-0 animate-in fade-in slide-in-from-right-4 duration-300">
                <div className="flex flex-col md:flex-row gap-12 items-center justify-center py-12">
                   ...
                   <div className="max-w-xs space-y-6">
                      ...
                      <div className="space-y-4">
                        <div className="flex items-center gap-3 text-sm text-green-500 font-black bg-green-500/5 p-4 rounded-2xl border border-green-500/10 uppercase tracking-widest">
                          <ShieldCheck className="h-5 w-5" /> Zero Fees
                        </div>
                        <div className="flex items-center gap-3 text-sm text-purple-500 font-black bg-purple-500/5 p-4 rounded-2xl border border-purple-500/10 uppercase tracking-widest">
              ...
              <TabsContent value="account" className="mt-0 animate-in fade-in slide-in-from-bottom-4 duration-300">
                <div className="bg-[#130b21] border border-white/10 rounded-[32px] p-20 text-center">
                  ...
                  <p className="text-white/40 font-bold">Advanced security and notification parameters coming soon.</p>
                </div>
              </TabsContent>
            </Tabs>
          </div>
