    ...
    <div className="min-h-screen bg-[#06030e] text-white overflow-x-hidden relative selection:bg-cyan-500/30">
      ...
      <main className="container mx-auto px-4 md:px-6 pt-48 pb-24 relative z-10">
        <div className="flex flex-col items-center text-center mb-16 md:mb-24">
          ...
          <p className="text-cyan-400/60 font-black uppercase tracking-[0.3em] md:tracking-[0.4em] text-[10px] md:text-xs mt-6">
            The Global XPR Network Top Earners
          </p>
        </div>

        {/* Podium Section - Responsive Layout */}
        <div className="flex flex-col md:flex-row items-center md:items-end justify-center gap-8 md:gap-6 mb-24 lg:mb-32">
          ...
          <div className="w-full max-w-sm md:w-72 order-2 md:order-1 animate-in slide-in-from-bottom-8 duration-700 delay-100">
            <div className="bg-[#120a21]/80 border-2 border-slate-400/30 rounded-[40px] p-6 md:p-8 text-center relative group hover:border-slate-400 transition-all shadow-[0_0_30px_rgba(148,163,184,0.1)]">
              <div className="absolute -top-4 md:-top-6 left-1/2 -translate-x-1/2 bg-slate-400 text-black font-black px-4 py-1 rounded-full text-[10px] md:text-xs uppercase tracking-widest shadow-lg">2nd Place</div>
              <div className="h-24 md:h-28 w-24 md:w-28 rounded-3xl mx-auto mb-6 border-4 border-slate-400/50 flex items-center justify-center text-3xl font-black bg-slate-900 overflow-hidden shadow-2xl relative">
              ...
              <p className="text-slate-400 font-bold mb-6 text-sm md:text-base">@{topThree[1].handle}</p>
              <div className="text-2xl md:text-3xl font-black text-slate-300">
                {topThree[1].totalTips.toLocaleString()} <span className="text-xs text-slate-500 font-black">TAB</span>
              </div>
            </div>
          </div>
          ...
          <div className="w-full max-w-sm md:w-80 order-1 md:order-2 animate-in zoom-in-95 duration-1000 scale-105 md:scale-110">
            <div className="bg-[#1a0f2e]/80 border-4 border-yellow-400/50 rounded-[48px] p-8 md:p-10 text-center relative group hover:border-yellow-400 transition-all shadow-[0_0_60px_rgba(250,204,21,0.2)] md:-translate-y-8 overflow-hidden">
              ...
              <p className="text-yellow-400 font-black mb-8 text-sm md:text-base">@{topThree[0].handle}</p>
          ...
          <div className="w-full max-w-sm md:w-72 order-3 animate-in slide-in-from-bottom-8 duration-700 delay-200">
            <div className="bg-[#120a21]/80 border-2 border-orange-700/30 rounded-[40px] p-6 md:p-8 text-center relative group hover:border-orange-700/60 transition-all shadow-[0_0_30px_rgba(194,65,12,0.1)]">
              <div className="absolute -top-4 md:-top-6 left-1/2 -translate-x-1/2 bg-orange-700 text-white font-black px-4 py-1 rounded-full text-[10px] md:text-xs uppercase tracking-widest shadow-lg">3rd Place</div>
              <div className="h-24 md:h-28 w-24 md:w-28 rounded-3xl mx-auto mb-6 border-4 border-orange-700/50 flex items-center justify-center text-3xl font-black bg-slate-900 overflow-hidden shadow-2xl relative">
              ...
              <p className="text-orange-400/60 font-bold mb-6 text-sm md:text-base">@{topThree[2].handle}</p>
              <div className="text-2xl md:text-3xl font-black text-orange-400/80">
                {topThree[2].totalTips.toLocaleString()} <span className="text-xs text-orange-800 font-black">TAB</span>
              </div>
            </div>
          </div>
        </div>

        {/* Scrolling List - Responsive Headers & Cards */}
        <div className="max-w-4xl mx-auto space-y-4">
          <div className="flex items-center justify-between px-6 md:px-8 py-4 text-cyan-400/40 font-black uppercase tracking-widest text-[9px] md:text-[10px]">
            ...
            <div className="flex items-center gap-6 md:gap-12">
              ...
              <span className="w-20 md:w-24 text-right">Total TAB</span>
            </div>
          </div>
          ...
          {others.map((creator, i) => (
            <div 
              key={creator.id}
              className="bg-white/5 border border-white/5 rounded-[24px] md:rounded-3xl p-4 md:p-6 flex items-center justify-between group hover:bg-white/10 hover:border-cyan-500/30 transition-all cursor-pointer"
            >
              <div className="flex items-center gap-4 md:gap-8">
                <span className="w-8 text-center font-black text-white/20 text-lg md:text-xl group-hover:text-cyan-400 transition-colors">#{i + 4}</span>
                <div className="flex items-center gap-3 md:gap-4">
                  <div className={`h-12 md:h-16 w-12 md:w-16 rounded-xl md:rounded-2xl ${creator.color} flex items-center justify-center text-base md:text-xl font-black border-2 border-white/10 overflow-hidden relative shadow-lg`}>
                    {creator.avatarImage ? (
                  ...
                  <div className="max-w-[100px] sm:max-w-none">
                    ...
                    <p className="text-xs md:text-sm text-white/40 font-bold group-hover:text-cyan-400/60 transition-colors truncate">@{creator.handle}</p>
                  </div>
                </div>
              </div>

              <div className="flex items-center gap-4 md:gap-12">
                <div className="hidden sm:flex items-center gap-2 text-white/40 font-bold text-xs md:text-sm">
                ...
                <div className="w-20 md:w-32 text-right">
                  ...
                  <span className="text-[9px] md:text-[10px] font-black text-white/20 ml-1 uppercase tracking-widest">TAB</span>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="mt-24 md:mt-32 p-8 md:p-16 rounded-[40px] md:rounded-[60px] bg-gradient-to-r from-magenta-600/20 via-purple-600/20 to-cyan-600/20 border-2 border-white/10 text-center relative overflow-hidden group">
          ...
          <Star className="h-10 w-10 md:h-12 md:w-12 text-yellow-400 mx-auto mb-6 animate-spin-slow" />
          <h2 className="text-3xl md:text-5xl font-black mb-4 tracking-tighter italic">WANT TO BE ON THIS LIST?</h2>
          <p className="text-white/60 text-lg md:text-xl max-w-xl mx-auto font-medium mb-10 leading-relaxed">
            Join the map, share your TipTab card, and let the network show their appreciation for your hustle.
          </p>
          <Button 
            onClick={() => setIsMembershipOpen(true)}
            className="bg-white text-black hover:bg-cyan-400 hover:text-black font-black text-lg md:text-2xl rounded-2xl md:rounded-[24px] h-16 md:h-20 px-8 md:px-12 shadow-[0_0_30px_rgba(255,255,255,0.2)] transition-all active:scale-95"
          >
            JOIN THE NETWORK
          </Button>
        </div>
